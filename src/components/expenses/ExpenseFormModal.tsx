...
export default function ExpenseFormModal({
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setImageError('Format non supporté. Utilisez JPG, PNG, WebP ou PDF.');
      return;
    }

    if (file.size > maxSize) {
      setImageError('Le fichier est trop volumineux (max 10MB).');
      return;
    }
  ...
  const processOCR = async (file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Échec de l\'analyse OCR');
      }
      ...
      setOcrError(error.message || 'Impossible d\'analyser l\'image. Veuillez remplir manuellement.');
    } finally {
      setIsProcessing(false);
