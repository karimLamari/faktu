...
import { FiRepeat, FiCalendar, FiClock } from 'react-icons/fi';
...
export default function ConvertQuoteModal({ open, onClose, quote, onConvert }: ConvertQuoteModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onConvert({ issueDate, dueDate });
      onClose();
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="flex items-center gap-2 text-sm font-semibold text-gray-300 mb-2">
                <FiCalendar className="w-4 h-4" /> Date d'émission de la facture *
              </label>
              <input
                type="date"
                value={issueDate}
            ...
            <div>
              <label className="flex items-center gap-2 text-sm font-semibold text-gray-300 mb-2">
                <FiClock className="w-4 h-4" /> Date d'échéance *
              </label>
              <input
                type="date"
                value={dueDate}
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button
              type="submit"
              disabled={loading}
              className="flex-1 h-12 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 shadow-md text-white"
            >
