 ...
 * 📊 Expense Parser
 * 
 * Parse les données de dépenses (fournisseur, montant, TVA, date, n° facture)
 * à partir du texte OCR extrait.
 * 
 * Supporte:
 * - Formats français et anglais
...
/**
 * Parse le texte OCR et extrait les données de dépense
 * @param ocrText - Texte extrait par OCR
 * @param provider - Provider utilisé (google-vision ou tesseract)
 * @returns Données parsées avec confiance
 */
export function parseExpenseData(
  ocrText: string,
  provider: 'google-vision' | 'tesseract' = 'tesseract'
): ParsedExpenseData {
  console.log(
    '🔍 Début du parsing - Texte reçu (200 premiers caractères):',
  ...
  let baseConfidence = provider === 'google-vision' ? 90 : 70;
  let confidence = 0;
  ...
  // Ajuster selon le provider
  confidence = Math.min(baseConfidence, confidence);
...
function extractAmount(text: string): number {
  ...
  const amountsWithContext = text.match(
    /([a-z\s]+)[\s:]*([0-9]+[,.]?[0-9]*)\s*€/gi
  );
    ...
    const prioritized = amountsWithContext
      .map((match) => {
        ...
        const amount = numMatch
          ...
          : 0;
        const priority = match.toLowerCase().includes('ttc')
          ? 3
          : match.toLowerCase().includes('total')
          ? 2
          : 1;
...
function extractVendor(text: string): string {
  ...
  for (let i = 0; i < Math.min(5, lines.length); i++) {
    ...
    if (
      line.match(
        /facture|ticket|reçu|siège|capital|r\.c\.s|tva|siret|date|n°|page|^[0-9]/i
      )
    ) {
      continue;
    }
    ...
    if (line.length >= 3 && line.length <= 40) {
      ...
      if (
        !line.match(/^[0-9]+\s+rue|^[0-9]+\s+avenue|^[0-9]+\s+boulevard/i)
      ) {
